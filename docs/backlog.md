...
> **Progreso: 5 de 8 HU.** ✅ HU-2.1 (capa de LLM) · ✅ HU-2.2 (personalidad vía system prompt) · ✅ HU-2.3 (modelo de datos de conversaciones) · ✅ HU-2.4 (endpoint de chat con streaming SSE) · ✅ HU-2.5 (contexto multi-turno y sesiones). **El núcleo conversacional está completo:** Rover conversa token a token, **recuerda** lo que se habló —con ventana por tokens y prefijo estable— y sus conversaciones se listan, se leen y se borran. Verificado de punta a punta contra DeepSeek real. Siguiente: **HU-2.6** (tool-calling y el clima como herramienta de referencia), que es lo que le da **datos en vivo**: hasta aquí Rover sabe lo que sabía su modelo, no lo que pasa hoy.
