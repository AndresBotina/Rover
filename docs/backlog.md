...
| **2** | El agente | Conversación con memoria, streaming SSE, sesiones y tool-calling con datos en vivo (clima primero) | **En curso** 🚧 |
...
- **Tipos de dominio propios** (`Message`/`Role`, `Completion`, `CompletionChunk`, `Usage`) para que el `dict` de DeepSeek no se pasee por el agente, el SSE y la persistencia. **Errores traducidos** a excepciones propias (`LLMRateLimited`, `LLMUnavailable`, `LLMAuthError`, `LLMQuotaExceeded`, `LLMBadRequest`, `LLMProviderError`, `LLMNotConfigured`, `LLMCapabilityUnavailable`), con el mismo contrato que las de `services/auth.py`: el mensaje es **seguro para el usuario** y el diagnóstico del proveedor (status, `code`, mensaje) viaja aparte, **solo para los logs**. La traducción es **por status**, nunca buscando palabras en el texto del error.
- **Streaming real**, no un extra: `stream(...)` es un async generator cuyos `text` son **deltas** (concatenarlos reconstruye la respuesta) y cuyo último trozo trae el `usage` — se pide `stream_options.include_usage`, porque si no el camino de streaming, que es el normal del producto, sería un **agujero ciego** en las métricas. El tipo de retorno es `AsyncGenerator` y no `AsyncIterator` **a propósito**: obliga a que exista `aclose()`, para que el endpoint SSE de la HU-2.4 cierre la conexión con el proveedor **en el momento** en que el usuario se va, en vez de dejarla colgando hasta que pase el recolector.
