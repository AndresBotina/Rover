...
> **Pendiente de verificar contra las APIs reales.** Todo lo anterior está probado sin red (modelo y clima mockeados). Falta la pasada end-to-end con las keys de verdad, que es la que confirma tres cosas que un mock no puede: que **DeepSeek decide** llamar a `get_weather` con la descripción que se le dio, que la key de OpenWeatherMap **está activa** (una recién creada responde 401 durante un rato) y que los `tool_steps` **quedan en Supabase** con el dato real dentro. La receta está en el README del backend, § Herramientas → Verificarlo en local: el camino corto es `uv run python -m scripts.check_tools`, que ejerce el turno completo sin servidor ni token e **imprime los `tool_steps`**; el largo es el `curl` con `--no-buffer`, el contraste contra la API del clima, el `grep -c tool_steps` en el historial y una consulta `psql` sobre `messages`.
