...
| **2** | El agente | Conversación con memoria, streaming SSE, sesiones y tool-calling con datos en vivo (clima primero) | **En curso** 🚧 (5/8) |
...
> **Progreso: 6 de 8 HU.** ✅ HU-2.1 (capa de LLM) · ✅ HU-2.2 (personalidad vía system prompt) · ✅ HU-2.3 (modelo de datos de conversaciones) · ✅ HU-2.4 (endpoint de chat con streaming SSE) · ✅ HU-2.5 (contexto multi-turno y sesiones) · ✅ HU-2.6 (maquinaria de tool-calling + clima). **Rover ya tiene datos en vivo:** conversa token a token, recuerda lo que se habló, y cuando necesita un dato que su modelo no tiene lo **consulta** — con el loop, el registro de herramientas y la primera herramienta concreta tras su propia abstracción. Los pasos intermedios se guardan y no se exponen. Siguiente: **HU-2.7** (más herramientas, repitiendo el molde) y **HU-2.8** (control de consumo).
