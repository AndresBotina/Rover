...
| **2** | El agente | Conversación con memoria, streaming SSE, sesiones y tool-calling con datos en vivo (clima primero) | **En curso** 🚧 (6/8) |
...
> **Progreso: 6 de 8 HU.** ✅ HU-2.1 (capa de LLM) · ✅ HU-2.2 (personalidad vía system prompt) · ✅ HU-2.3 (modelo de datos de conversaciones) · ✅ HU-2.4 (endpoint de chat con streaming SSE) · ✅ HU-2.5 (contexto multi-turno y sesiones) · ✅ HU-2.6 (maquinaria de tool-calling + clima). **Rover ya tiene datos en vivo:** conversa token a token, recuerda lo que se habló, y cuando necesita un dato que su modelo no tiene lo **consulta** — con el loop, el registro de herramientas y la primera herramienta concreta tras su propia abstracción. Los pasos intermedios se guardan y no se exponen, y está **verificado de punta a punta contra las APIs reales**. Quedan dos, y no son simétricas: la **HU-2.7** (más herramientas) es **trabajo abierto** —el molde ya está fijado, así que cada tool nueva es una repetición y se concreta al priorizarla, no antes—, mientras que la **HU-2.8** (control de consumo) es la que de verdad falta para poder abrir esto a alguien. La v1 mínima de la épica **puede cerrarse con la 2.8**.
...
> **El modelo no recuerda nada entre llamadas.** Rover recuerda porque en cada turno se le vuelve a contar la conversación — y eso se paga **cada vez**. De ahí que la pregunta de esta HU no sea *"¿cuánto cabe?"* sino **"¿cuánto conviene?"**.

**Criterios de aceptación (como se construyó):**
- **Reconstrucción del contexto:** antes de llamar al modelo se relee lo guardado (`load_history` sobre `select_conversation_messages`, que hereda el `ORDER BY sequence` de la HU-2.3) y se recorta. **Stable-prefix-first**: el system prompt sigue yendo **aparte y primero** —lo antepone la capa de la HU-2.1—, y lo variable detrás; es lo que hace que el caché de DeepSeek muerda (98 % de la entrada servida de caché, medido en la HU-2.2). El recorte vive en `trim_to_budget`, separado de `build_context`, que solo traduce: así la parte con reglas se prueba **a solas**, sin base de datos ni endpoint de por medio.
- **Ventana por TOKENS, no por número de mensajes.** Diez turnos pueden ser 300 tokens o 30.000 según lo que se haya escrito: un tope por turnos deja el costo —y el riesgo de reventar el contexto— a merced de lo larga que fuera una respuesta. Y **no es solo costo**: con entradas largas el modelo atiende peor a lo que queda por el medio (*lost in the middle*), así que mandar todo el historial "por si acaso" **empeora las respuestas** además de encarecerlas. `ROVER_CHAT_CONTEXT_TOKEN_BUDGET` (4.000) acota el **historial**; el system prompt (constante, casi siempre cacheado) y el mensaje nuevo (innegociable) van encima.
- **Se recorre hacia atrás y se PARA en el primero que no cabe** (`break`), sin seguir buscando alguno más viejo que sí quepa. Saltarse un mensaje del medio dejaría un **agujero** en una lista que el modelo lee como secuencia continua, y acabaría **atribuyendo a una pregunta la respuesta de otra**. Se conserva lo reciente porque es lo que da sentido a la pregunta de ahora; lo viejo es lo prescindible.

**Decisiones de diseño:**
- **Conteo de tokens: aproximación por caracteres, PESIMISTA** — `4 + ceil(len(texto) / 3.0)` por mensaje, donde los 4 son el envoltorio del formato del proveedor (rol y delimitadores), que se paga aunque el texto sea de una palabra; sin ese sumando, una conversación de muchos turnos cortos se subestimaría justo donde más envoltorios hay. **Por qué no un tokenizador:** DeepSeek **no publica uno instalable** para su V4, y `tiktoken` es otro BPE — daría una cifra precisa **de un modelo distinto** (precisión falsa) y además **descarga un fichero de modelo en el primer uso**: una llamada a la red en el arranque, dentro de un CI que corre sin secretos y de un plan free que se duerme. **Por qué pesimista:** los dos errores **no cuestan lo mismo** — subestimar manda de más y el proveedor responde **400** (rompe el turno); sobreestimar manda de menos y **solo se pierde memoria vieja** (degradación suave). Contar a 3,0 caracteres por token cuando el español real ronda 3,5-4,0 es **~20 % de sobreestimación deliberada**. Lo que hace defendible el número no es el número: es que el margen es **medible** —la instrumentación de la HU-2.1 ya registra el `llm_input_tokens` REAL de cada llamada, así que la serie dice si el 3,0 sobra o falta— y **ajustable por config** (`ROVER_CHAT_CONTEXT_CHARS_PER_TOKEN`), no por código.
- **Los `tool_steps` NO se reenvían al contexto, y no es un pendiente de la HU-2.6.** Tres razones que no cambian cuando aquella HU los llene: (1) **están caducados** — el JSON crudo de una API del clima era cierto *aquel* momento, y reenviarlo tres turnos después le enseña al modelo como **vigente** algo que ya no lo es, que es peor que no tenerlo; (2) **cuestan mucho y aportan poco** — una respuesta de API puede ocupar más tokens que toda la conversación, para decir algo que el turno del asistente ya resumió en una frase; (3) **la conclusión ya vive en `content`** — lo que hay que recordar de un turno con herramientas es lo que Rover **concluyó**, no cómo lo averiguó. Lo que sí necesita el formato de cable de la tool (`role="tool"`) es el **loop del turno en curso**, que la HU-2.6 armará en memoria y tirará al terminar: por eso **`build_context` no cambia** con aquella HU. Si algún día conviene que quede rastro de una herramienta en el historial, la extensión es una **nota sintética corta** ("consultó el clima de Bogotá"), **nunca el JSON**.
- **El historial nunca empieza por una respuesta huérfana.** Si el corte parte un par pregunta/respuesta, el `assistant` que quedaría primero **se descarta**: el modelo lo lee como algo que Rover dijo **por su cuenta**, y lo que suele haber ahí es media respuesta a algo invisible — que invita a "continuarla" en vez de atender lo que se está preguntando ahora; además, abrir con `assistant` es una forma rara en un formato que asume turnos alternos. El costo es tirar un mensaje que sí cabía: barato, y **el borde deja de existir** en vez de quedar dependiendo de dónde caiga el corte. Si al quitarlo no queda nada, el turno se comporta como el primero de la conversación — lo honesto: con ese presupuesto **no hay memoria que reenviar**.
- **`updated_at` ya refleja la actividad** (deuda que dejaron anotadas la HU-2.3 y la HU-2.4). Cada mensaje **toca su conversación** en la MISMA transacción, con `func.now()` para que el reloj de la **base** siga siendo la única fuente de esa columna — mezclarlo con el del proceso permitiría que un `updated_at` quedara **antes** de su `created_at` con solo un poco de deriva entre relojes. Sin este UPDATE el `onupdate` no saltaba nunca (añadir un mensaje no actualiza esa fila) y el listado habría ordenado por fecha de **creación**, con una conversación vieja que se retoma sin subir: **una lista mal ordenada, no un error**. El orden **desempata por `id`**, porque `updated_at` no es único: sin desempate la lista se baraja sola entre dos refrescos, y la paginación por keyset —que es la extensión natural, con `updated_at` de cursor— se saltaría filas o las repetiría en el borde de página.

**Endpoints de sesiones** (`app/api/v1/sessions.py` — mismo router, prefijo, cuota y tag que `/v1/chat`, pero en su propio módulo: comparten el sitio, no el problema; allí hay un stream y un proveedor, aquí tres lecturas y un soft-delete):
- **`GET /v1/chat/sessions`** — las conversaciones **vivas** del usuario, por actividad reciente, **solo cabeceras** (id, título, timestamps). Devolver los mensajes aquí convertiría pintar una barra lateral en descargar toda la cuenta. `limit` 1-100 (default 50), y la respuesta va **envuelta en un objeto** —no un array pelado— para poder ganar campos de paginación sin romper a ningún cliente publicado. Sin cursor hoy a propósito: fijarlo sería decidir el contrato antes de saber si la web quiere scroll infinito o una pantalla de "ver todas".
- **`GET /v1/chat/sessions/{id}`** — el historial en orden, **solo texto conversacional** (`role`, `content`, `sequence`, `created_at`) y **nunca los `tool_steps`**. La frontera está en el **TIPO**: `SessionMessage` no declara ese campo, así que no hay filtro que alguien pueda olvidar — la misma defensa que `ProfileUpdate` monta contra `plan` (HU-1.10b), y aquí pesa más porque esos pasos llevan respuestas crudas de APIs de terceros, exactamente la clase de dato que nadie revisa hasta que aparece en la pantalla de alguien.
- **`DELETE /v1/chat/sessions/{id}`** — soft-delete → **204**. Es **un solo UPDATE condicional** (`id AND user_id AND deleted_at IS NULL`) y no leer-y-luego-escribir: la propia sentencia comprueba la pertenencia, así que no hay ventana entre la lectura y la escritura, y `rowcount == 0` cubre **los tres casos del 404** de una vez. **Borrar dos veces responde 404 la segunda**, no 204: es la tensión conocida entre idempotencia y no filtrar, y gana no filtrar — un 204 sobre una ya borrada diría *"ese id existió y era tuyo"*. Para el cliente el desenlace es el mismo (no está).
- **Ajena, inexistente o borrada → `404` indistinguible** en los tres endpoints, igual que en la HU-2.4 y por lo mismo: colapsan dentro de la **consulta** (`select_live_conversations` ya filtra `deleted_at IS NULL`), no en un `if` que un refactor pueda separar.

**Resto de los criterios:**
- **`@rover/shared`:** tipos, type guards y métodos para **listar, leer el historial y borrar**. El historial es **solo texto por TIPOS**: `ChatSessionMessage` no declara `tool_steps`, así que un cliente no puede ni esperarlos ni pintarlos por descuido — el compilador se lo impide antes de que exista la pantalla donde aparecerían. `deleteChatSession` documenta que un `not_found` en un borrado se trata como "ya no está", no como un fallo que mostrar.
- **Tests: 42 nuevos en el backend (357) y 14 en `@rover/shared` (84).** Dos capas a propósito: **el recorte como función pura** (el corte que parte un par cabe en tres líneas, sin base ni endpoint) y **el turno completo contra el endpoint afirmando sobre lo que RECIBIÓ el modelo** —el doble guarda los mensajes que le llegaron— en vez de deducirlo de la respuesta. Un test **cierra el círculo con la HU-2.4**: una respuesta abandonada a mitad **no reaparece** en el contexto del turno siguiente, porque nunca se persistió. El DELETE se verifica entero: no listada, `GET` 404, `POST /v1/chat` con ese id 404, y las filas **siguen en la base** (que es lo que hace útil el soft-delete).
- **Verificado end-to-end.** Con `uvicorn` real y un proveedor que **hace eco del contexto**, el turno 2 recibe el turno 1 completo y en orden, retomar una conversación vieja la sube al principio de la lista, y el detalle devuelve exactamente las cuatro claves conversacionales. Contra DeepSeek real: un segundo turno que **no nombra el destino** responde con coherencia sobre lo del primero, y ese mismo mensaje **sin `conversation_id`** no tiene de dónde saberlo — la prueba negativa vale tanto como la positiva. El `llm_input_tokens` crece turno a turno mientras el `llm_cache_hit_ratio` se mantiene alto: la memoria cuesta, y el prefijo estable sigue delante.

**Tareas técnicas (como se hizo):** `estimate_tokens` / `trim_to_budget` / `load_history` en `app/services/chat.py` · `build_context` recibiendo ya el historial recortado · el UPDATE de `updated_at` dentro de `append_message` · desempate por `id` en `select_live_conversations` · `app/api/v1/sessions.py` con los tres endpoints y sus modelos de respuesta · dos variables nuevas en `Settings` y en `.env.example` · `packages/shared/src/types/session.ts` y los tres métodos del cliente · `tests/test_chat_context.py`, `tests/test_chat_sessions.py`, `session.test.ts` y ampliación de `client.test.ts` y `test_openapi.py` · README del backend (§ La memoria de Rover).

> **Lo que esta HU cierra:** con la memoria multi-turno, el **núcleo conversacional de la épica está completo** — Rover conversa, recuerda y sus conversaciones se gestionan. Lo que falta (HU-2.6 a 2.8) le añade **datos en vivo** y **control de consumo**, no una forma distinta de conversar.
...
> **Lo que esta HU construye es el MOLDE, y el clima es quien lo valida.** El objetivo doble era dejar un patrón reutilizable para las tools futuras (lugares, búsqueda web) y probarlo con la más simple. Por eso la maquinaria (`app/services/tools/`) **no sabe qué es el clima** y el proveedor de clima (`app/services/weather/`) **no sabe qué es una herramienta**: se encuentran en un archivo de 130 líneas, `tools/weather.py`.

**Criterios de aceptación (como se construyó):**
- **Registro e interfaz de herramientas.** `Tool` es un `Protocol` (nombre, descripción, JSON Schema de parámetros, etiqueta de estado y un `run` async) — se cumple **por estructura**, sin heredar de nada, igual que `LLMProvider` (HU-2.1) y `RateLimitStore` (HU-1.7). `ToolRegistry` las guarda por nombre y las traduce a los `ToolSpec` que la capa de LLM le presenta al proveedor. **Añadir una herramienta son dos pasos y ninguno toca el loop:** implementar el `Protocol` y registrarla en `_catalogo_por_defecto`. Un nombre repetido es un `ValueError` al registrar, no un reemplazo silencioso: si no, el modelo pediría una y se ejecutaría otra, un fallo que se manifiesta como "la respuesta no tiene sentido" y nunca como una excepción.
- **La capa de LLM aprendió tool-calling sin filtrar la forma de DeepSeek.** Tipos de dominio nuevos: `ToolSpec` (la declaración), `ToolCall` (el modelo pide), `ToolCallDelta` (un fragmento por el stream) y `ToolCallAccumulator` (que los pega). `Completion` y `CompletionChunk` ganaron `tool_calls` como campo **independiente** del texto, no como unión: "texto O tools" obligaría a un `isinstance` en cada llamador para descubrir que en realidad puede haber **ambos** (una frase de preámbulo antes de consultar, que es un caso real y se transmite). El envoltorio `{"type":"function", …}` y el `tool_call_id` del formato de cable se quedan en `deepseek.py`.
- **`ToolCall.arguments` viaja CRUDO, sin parsear.** Es deliberado: un modelo manda JSON roto con una frecuencia incómoda, y si el tipo exigiera un `dict` el parseo tendría que ocurrir dentro del proveedor — el sitio con **menos contexto** para decidir qué hacer cuando falla. Dejándolo crudo, quien decide es el loop, que puede devolverle el error al modelo para que se corrija.
- **El acumulador vive en `base.py`, no en el proveedor.** En streaming una petición llega repartida (primero id y nombre, después los argumentos carácter a carácter, varias simultáneas distinguidas por `index`). Reconstruirla es el mismo problema para cualquier proveedor OpenAI-compatible, así que se resuelve una vez y se hereda.
- **El loop: cero, una o varias llamadas, con el mismo código.** Cero → el modelo responde directo y el turno es **byte a byte** el de la HU-2.4 (el camino de la mayoría). Una → vuelta 1 pide, se ejecuta, vuelta 2 redacta. Varias → en la misma vuelta (se ejecutan en orden, cada resultado vuelve con el `tool_call_id` de **su** petición) o encadenadas en vueltas distintas.
- **El loop es un OBJETO, no una función.** El turno produce **dos** cosas con destinos opuestos: el flujo de eventos, que se consume **mientras ocurre** y sale al cliente, y los `tool_steps`, que se leen **al final** y no salen nunca. Devolver ambas desde un generador obligaría a mezclarlas en el mismo canal — exactamente la confusión que la HU-2.3 evitó poniéndolas en columnas distintas de la tabla. Con un objeto, `run()` emite lo público y `.steps` se lee después: la frontera está en la forma del tipo, no en la disciplina de quien lo consuma.
- **El texto final va por SSE como siempre**, y el turno del asistente que **pidió** las herramientas se reinyecta al contexto con sus `tool_calls` — sin él, el mensaje de rol `tool` que viene después no responde a nada y el proveedor devuelve 400.
- **Los pasos intermedios se persisten y NO se exponen.** Cada ejecución deja un paso en `messages.tool_steps` (HU-2.3): herramienta, argumentos, resultado o error, causa técnica, duración y en qué vuelta ocurrió. El `content` sigue siendo solo texto conversacional, y los pasos no salen ni por el stream, ni por `GET /v1/chat/sessions/{id}`, ni al contexto de los turnos siguientes.
- **Degradación elegante en los cuatro caminos de fallo** (herramienta inexistente, argumentos inválidos, API caída, bug nuestro) — ver abajo.
- **Tests con tool y modelo mockeados**: ciclo completo, camino de fallo, no exposición y límite del loop.

**Decisiones de diseño:**
- **El límite del loop es 3 vueltas (`ROVER_TOOL_LOOP_MAX_ITERATIONS`), y acota COSTO, no un cuelgue.** El camino normal gasta 2 (pedir → redactar); la tercera deja sitio a un encadenamiento legítimo —consultar el clima de una ciudad, ver que se preguntaba por dos, consultar la otra— sin dejar hueco a un modelo terco. Lo que hace que el límite importe no es que la petición se quede colgada: es que **cada vuelta es una llamada facturada con un contexto que CRECE** (arrastra la petición y el resultado de la anterior), así que un loop descontrolado no se nota como lentitud sino como una **factura**. Y al llegar al límite **no se corta la conversación**: la última llamada se hace **sin ofrecer herramientas**, así que al modelo no le queda más que redactar en prosa con lo que ya tiene. La diferencia es "el usuario recibe una respuesta escrita con datos incompletos" en vez de "el usuario recibe un error", y **no cuesta nada** porque esa llamada había que hacerla igual.
- **Las tools se ofrecen SIEMPRE, todas y en el mismo orden.** Filtrar por "parece pertinente" exigiría un clasificador: o una lista de palabras clave (que falla con *"¿me llevo chaqueta a Bogotá?"*) o **otra llamada a un modelo**, más latencia y más costo que la herramienta que se quería ahorrar. Y **rompería el prefijo estable**: las definiciones viajan en el mismo cuerpo que el system prompt, así que un catálogo que cambia entre turnos tira el caché automático de DeepSeek en cada llamada — los 98 % de entrada cacheada que midió la HU-2.2 son lo que se estaría gastando. La degradación que señala la investigación aparece con catálogos **grandes** (decenas de herramientas parecidas); con una, el modelo acierta. Por eso `specs()` devuelve las declaraciones **ordenadas por nombre** y no por orden de inserción: un refactor que cambiara el orden del catálogo invalidaría el caché **sin que ningún test se pusiera rojo**. Se revisa cuando el catálogo pase de ~10, y la respuesta entonces será **agrupar** (un "buscador" que enruta por dentro), no filtrar.
- **Durante la fase de tool el cliente SÍ ve algo: un evento `status`** con el nombre de la herramienta y una frase hecha ("Consultando el clima…"). Se decidió emitirlo, y no callar, porque esa fase abre un **silencio de varios segundos** —una llamada al modelo que no imprime nada, más una llamada HTTP a un tercero— justo en un producto cuya promesa es ver el texto aparecer palabra a palabra; sin señal, la pausa es indistinguible de un cuelgue y la reacción natural es recargar (que además tira la respuesta que venía en camino). **Lo que NO viaja:** los argumentos (los escribe el modelo y pueden llevar una interpretación equivocada de la pregunta — enseñarlos convierte un detalle interno en algo que el usuario tiene que juzgar), el resultado (respuesta cruda de un tercero) y **el estado de una herramienta que no existe** (anunciar "consultando fantasía…" sería exponer una alucinación como si fuera una capacidad del producto).
- **Fallo de una herramienta: se le devuelve al MODELO, no se degrada por nuestra cuenta.** Los cuatro caminos —nombre inventado, argumentos que no son JSON, API caída, excepción no prevista— terminan igual, en un mensaje de rol `tool` que cuenta qué pasó. Tres razones: (1) **la personalidad ya sabe hacerlo** — el prompt de Rover (HU-2.2) le pide ser honesto cuando no tiene un dato, así que con el error en la mano escribe *"no pude mirar el clima ahora mismo, pero en Bogotá en esta época…"* en el tono de la conversación **y en su idioma**, cosa que una frase enlatada no puede; (2) **el turno no era solo la herramienta** — *"¿qué hago en Medellín y qué tal el tiempo?"* merece la mitad que sí se puede contestar, y una frase enlatada la tiraría entera; (3) **a veces el error es corregible** — *"no encontré ningún lugar llamado X"* le permite reintentar con el nombre bien escrito, que es una vuelta más del loop y una respuesta buena en vez de una excusa. La causa REAL (status del proveedor, traza si fue un bug) va al log y a `tool_steps`; al modelo solo le llega una frase segura.
- **Que TODA petición se conteste no es elegancia, es un requisito del formato de cable.** Si el turno del asistente viaja con tres `tool_calls` y solo se contestan dos, el proveedor responde **400** y se cae la conversación entera: un fallo de una herramienta convertido en un fallo del chat, que es exactamente lo que esta HU existe para evitar. Un test lo fija con una llamada buena, una a herramienta inexistente y una que falla, comprobando que salen **tres** mensajes `tool` con sus tres ids.
- **La validación de argumentos es un subconjunto de JSON Schema escrito a mano, sin dependencia nueva.** Comprueba lo que de verdad falla: que sea JSON, que sea un objeto, que estén los obligatorios y que los tipos cuadren (con `bool` excluido de los numéricos — en Python es un `int`, y un `true` donde va una temperatura es un error, no un 1). Traer `jsonschema` sería una dependencia y su superficie para sustituir cuatro `if` sobre esquemas de diez líneas que escribimos nosotros; está en **una función** para que cambiarla el día que los esquemas crezcan sea reemplazar su cuerpo. **Los campos de más se ignoran** en vez de romper: un modelo que añade un argumento que no pedimos está siendo generoso, y rechazarlo gastaría una vuelta entera del loop para nada.
- **El clima, tras su abstracción, con la geocodificación DENTRO.** `WeatherProvider.current(location)` recibe un **nombre** y devuelve el clima; que OpenWeatherMap necesite coordenadas y haya que geocodificar primero es problema suyo. Si la interfaz fuera `geocode()` + `weather(coords)` estaría **copiando la forma de la API de OpenWeatherMap**, y el día que otro proveedor lo resolviera en una llamada —o en tres— habría que reescribir a quien las orquesta. El resultado que ve el modelo va **recortado y con la unidad en el nombre de la clave** (`temperature_c`, `wind_kph`): cuesta tres caracteres de prompt y elimina una clase entera de error, además de no pagar como entrada los treinta campos que el JSON crudo trae y a nadie le importan.
- **`units=metric` está fijado en el CÓDIGO, no en la config, y la conversión de viento va clavada con tests.** `wind.speed` llega en **m/s** y sale en km/h (`× 3.6`). Que la unidad no sea configurable es deliberado: con `imperial` el viento vendría en **mph** y ese mismo `× 3.6` daría un número **un 61 % alto que seguiría pareciendo viento** — un fallo mudo, al contrario que la temperatura, donde perder `units` daría Kelvin y se vería al instante. Por eso el factor está fijado **contra valores calculados a mano** (`5,66 → 20,4`, `7,6 → 27,4`, y 5,66 interpretado como nudos o mph daría 10,5 o 9,1, que no pasan): es la conversión más fácil de romper sin que se note, porque cualquier número plausible en km/h pasa la vista. Ver más abajo la nota del reporte de bug falso, que es de dónde salió esta disciplina.
- **Los errores del clima son TRES y no uno.** `WeatherNotFound` (el lugar no existe → el modelo puede pedir que se concrete), `WeatherUnavailable` (caído, timeout, sin cuota, **key todavía sin activar**) y `WeatherNotConfigured` (falta la key). Colapsarlos perdería justo la distinción que hace buena la respuesta degradada: *"no encontré ese lugar, ¿me lo concretas?"* y *"el servicio no responde ahora mismo"* son dos conversaciones distintas. El **401 por key recién creada** cae en `Unavailable` a propósito —desde fuera es indistinguible de una revocada y en los dos casos toca lo mismo— pero deja en el log una línea que **lo dice explícitamente**, porque si no el síntoma manda a revisar el código en vez de a esperar diez minutos.
- **`ROVER_WEATHER_API_KEY` NO es obligatoria en producción, y sin ella la herramienta NO SE REGISTRA.** El modelo no puede pedir lo que no se le ofrece, así que Rover conversa igual (solo que sin datos del tiempo) y queda un warning en el log — mismo criterio que `ROVER_CORS_ORIGINS` (HU-1.11): negarse a arrancar castigaría todo lo demás por una capacidad opcional. Ofrecerle al modelo una herramienta que **solo puede fallar** cuesta tokens en cada llamada para acabar en una degradación evitable, y además le enseña a prometer un dato que no va a llegar. Misma lógica que `get_llm_provider(Capability.VISION)`: mejor que no exista a que exista rota.
- **El punto de cambio a Open-Meteo, explícito y trivial.** Se eligió OpenWeatherMap por dos razones prácticas y no por ser mejor: **geocoding y clima en el mismo servicio y con la misma key** (un proveedor que dar de alta, un sitio donde mirar cuota) y plan gratuito suficiente. **Open-Meteo es la alternativa sin key** y su API tiene la misma forma de dos pasos, así que el cambio completo son tres pasos: escribir `app/services/weather/openmeteo.py` cumpliendo el `Protocol` (~80 líneas: dos GET y un parseo), cambiar `_construir_proveedor()` en `weather/registry.py` y borrar la variable del entorno. **No se toca el loop, ni el endpoint, ni la definición de la herramienta que ve el modelo, ni un solo test del ciclo** — porque **ninguno importa el módulo del proveedor**; el único que lo hace es ese `registry.py`, y hay exactamente **una** aserción en el repo que habría que actualizar (`test_con_key_se_construye_el_de_OpenWeatherMap`). El **uso comercial** de cualquiera de los dos (los términos del plan gratuito de OpenWeatherMap, la licencia de atribución de Open-Meteo) se revisa en la **Épica 5**, con el resto de las APIs de pago: hoy es una decisión técnica y reversible, no un compromiso.
- **El system prompt cambió, y era obligatorio.** Decía *"no tienes acceso a información en tiempo real"* — con eso delante, un modelo obediente **se niega a usar la herramienta**. Ahora hay una sección de **política de herramientas** (úsalas cuando las tengas, no las anuncies, si fallan dilo con naturalidad, lo que devuelven es de **ese momento** y no un pronóstico) y la de Honestidad quedó acotada a *"fuera de lo que puedas consultar con una herramienta"*. **Las herramientas NO se enumeran en el prompt**: eso crearía dos fuentes de verdad y, el día que una no se registre por falta de key, el prompt seguiría prometiéndola. La lista real viaja en su propia clave del cuerpo; el prompt solo lleva la **política**. Queda escrito en `prompts/README.md`.
- **El contrato del stream creció sin romper a nadie.** `ChatStatusEvent` en `@rover/shared` con su type guard. Los clientes ya publicados **lo descartan en silencio** porque `isChatStreamEvent` es estricto (rechaza lo que no conoce) y `parseChatStreamEvent` devuelve `null` en vez de lanzar: un evento nuevo se ignora, nunca rompe el parseo. Hay un test que lo fija reconstruyendo el guard "viejo".
- **`_primer_trozo` pasó a ser `_primer_evento`, con un costo asumido.** El endpoint sigue pidiendo el primer evento **antes** de abrir la respuesta HTTP, que es lo que mantiene la separación entre el fallo (1) —status HTTP de verdad— y el (2) —evento SSE— de la HU-2.4. Pero cuando el modelo decide usar una herramienta, la primera llamada **no produce texto**, así que ahora esa espera incluye una **llamada entera** al modelo y no solo su primer token. Se asume por lo mismo que entonces: a cambio, "el proveedor está caído" sigue siendo un error HTTP normal, que es como los clientes ya saben tratarlo.
- **La instrumentación gana la serie `app.tools`**, con la misma forma que `app.llm`: una línea por ejecución con nombre, resultado, duración y vuelta, **sin una palabra de la conversación**. Es con lo que se responderá "¿cuánto falla el clima?" sin leer conversaciones — y, junto a las dos líneas `app.llm` que ahora deja un turno con herramientas, la materia prima del control de consumo de la HU-2.8.

**Resto de los criterios:**
- **Tests: 104 nuevos en el backend (461) y 2 en `@rover/shared` (86).** Repartidos en tres módulos por lo que prueban, no por dónde viven: `test_tools.py` (43) es la **maquinaria en aislamiento** —los mismos tests valdrían con la herramienta de lugares—, `test_chat_tools.py` (13) es el **recorrido completo** por el endpoint y la base, y `test_weather.py` (28) es el **proveedor**, ejercido entero contra `httpx.MockTransport` (los dos GET encadenados, la query, el parseo, la conversión de unidades y la traducción de errores) porque un mock del método `current` no probaría nada de eso. El doble del modelo emite las peticiones de herramienta **fragmentadas**, como el real: si las mandara enteras, el acumulador —que es justo la pieza que reconstruye eso— no se ejercería nunca. **La no exposición se prueba con un señuelo** dentro del resultado de la herramienta: si esa cadena aparece en el cuerpo del stream o del historial, el test se pone rojo. Y hay un test de la **frontera de la abstracción**: que del proveedor de clima no salga nada con forma de OpenWeatherMap (`cod`, `icon`, ids), que es lo que hace local el cambio de proveedor.
- **Los 357 tests previos siguen verdes** sin cambiarles una aserción. Lo único que se tocó de ellos son los **dobles del `Protocol`** (que ganaron el parámetro `tools`) y el test del aborto del cliente, que ahora ejerce el **loop** en vez del stream pelado — porque es lo que consume el endpoint, y el `aclose()` tiene que atravesarlo hasta la conexión con DeepSeek.
- **Tooling de desarrollo, no producto.** `scripts/check_tools.py` ejerce el turno completo contra las APIs reales sin servidor ni token e **imprime los `tool_steps`** —el único sitio donde se enseñan, porque por el stream y por la API no salen—, y `scripts/chat.py` encadena login y chat por SSE con las credenciales de dev del `.env` (`ROVER_DEV_TEST_*`), para probar el agente escribiendo una frase en vez de dos `curl`. Ninguno corre en CI ni lo importa nadie: hablan con la API por HTTP como cualquier cliente.
- **Ruff, mypy estricto, pytest, ESLint, Prettier y tsc en verde.**

**Tareas técnicas (como se hizo):** tipos de tool-calling en `app/services/llm/base.py` (`ToolSpec`, `ToolCall`, `ToolCallDelta`, `ToolCallAccumulator`, campos en `Message`/`Completion`/`CompletionChunk`) · traducción al formato de cable y parseo de `tool_calls` (completo y por fragmentos) en `deepseek.py` · `app/services/tools/` (`base.py`, `registry.py`, `loop.py`, `weather.py`) · `app/services/weather/` (`base.py`, `openweathermap.py`, `registry.py`) · `ToolLoop` enchufado en `app/api/v1/chat.py` con el evento `status` y `tool_steps` en la persistencia · `tool_steps` en `chat.append_message` · cinco variables nuevas en `Settings` y en `.env.example` · política de herramientas en `prompts/rover.md` y su regla en `prompts/README.md` · `ChatStatusEvent` y su guard en `@rover/shared` · `tests/test_tools.py`, `tests/test_chat_tools.py`, `tests/test_weather.py`, ampliación de `tests/test_llm.py` y `chat.test.ts`, fixture de aislamiento en `conftest.py` · `scripts/check_tools.py` (comprobación manual contra las APIs reales) · README del backend (§ Herramientas).

> **Verificado end-to-end contra las APIs reales.** Las tres cosas que un mock no puede confirmar, confirmadas: **DeepSeek decide** llamar a `get_weather` con la descripción que se le dio (no hubo que empujarlo desde el prompt), la herramienta **consultó el clima real** de Bogotá y el modelo tejió los datos concretos en la respuesta —temperatura, humedad y viento, con la conversión correcta—, y los **`tool_steps` quedaron persistidos** en el mensaje del asistente **sin aparecer en el stream**. El proveedor de clima se comprobó además a solas contra la key de verdad (geocoding → clima → tipo de dominio), con los cuatro valores coincidiendo con la fuente en el mismo instante. La receta está en el README del backend, § Herramientas → Verificarlo en local: el camino corto es `uv run python -m scripts.check_tools`, que ejerce el turno completo sin servidor ni token e **imprime los `tool_steps`**; el largo es el `curl` con `--no-buffer`, el contraste contra la API del clima, el `grep -c tool_steps` en el historial y una consulta `psql` sobre `messages`.

> **Cómo verificar el clima SIN llevarse un susto — el reporte de bug falso del 25-08-2026.** Se reportó que Rover decía **27 km/h** de viento donde la fuente marcaba **5,66 m/s** (= 20,4 km/h), con un factor implícito de 4,77 que no corresponde a ninguna conversión. **La conversión estaba bien**: `wind_kph = wind.speed × 3.6` con `units=metric` en la petición, comprobado contra la API real en el mismo instante — el punto que consulta Rover marcaba **7,6 m/s → 27,4 km/h**, exactamente lo que dijo. Lo que fallaba era **la receta de verificación del README**, escrita en esta misma HU: mandaba contrastar contra `?q=Bogota`, y ese es **otro punto** (4,61 / -74,08) que el de `/geo/1.0/direct` que Rover geocodifica (4,71 / -74,07). A diez kilómetros el viento es distinto **de verdad**, así que la receta fabricaba una discrepancia real que parecía un error de conversión. La temperatura también difería (16,21 vs 16,59) y **nadie lo notó**, porque los dos números eran igual de creíbles — que es justo por qué el viento fue el que saltó. Corregido: la receta ahora compara **en las mismas coordenadas**, con la conversión hecha a la vista en el `jq`, y avisa del desfase temporal. Y la conversión quedó clavada con valores calculados a mano (5,66 → 20,4 entre ellos) para que la pregunta tenga una respuesta en el repo y no en una calculadora. **La regla, para la próxima herramienta que consulte un dato geolocalizado:** se compara contra las **mismas coordenadas** que usó la tool y en el **mismo instante**, o no se compara — y `scripts.check_tools` imprime el valor exacto que recibió el modelo, que es la vía sin ambigüedad.

> **Lo que esta HU deja montado para la HU-2.7.** Cada herramienta nueva es una repetición del molde, no un diseño: implementar `Tool` y registrarla. Lo que sí tendrá que decidirse allí, y no aquí, es **cuándo dejan de ofrecerse todas** (el catálogo grande es donde aparece la degradación) y si alguna necesita su propia abstracción de proveedor — la de lugares seguro que sí, y por la misma razón que el clima: es API de pago y la elección se revisa por costo y lock-in.
