...
>
> - **Es config (`ROVER_LLM_THINKING`), no una constante**, porque el router por dificultad —diferido— querrá justo lo contrario para lo que sí lo vale (un itinerario de tres ciudades con fechas y presupuesto). Misma filosofía que `ROVER_LLM_MODEL`: reactivarlo es cambiar entorno, no código. Un valor desconocido **corta el arranque** en vez de caer en silencio al modo caro.
