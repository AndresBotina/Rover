...
| **2** | El agente | Conversación con memoria, streaming SSE, sesiones y tool-calling con datos en vivo (clima primero) | **En curso** 🚧 (3/8) |
...
> **Progreso: 3 de 8 HU.** ✅ HU-2.1 (capa de LLM) · ✅ HU-2.2 (personalidad vía system prompt) · ✅ HU-2.3 (modelo de datos de conversaciones — primera de la épica que toca base de datos, y la que estrenó el patrón de fixtures limpio de la HU-1.13; migración aplicada y verificada contra Supabase). Siguiente: **HU-2.4** (endpoint de chat con streaming SSE), que es donde las dos tablas empiezan a llenarse de verdad.
